...
> - **React Compiler is enabled** in this project (`reactCompiler: true` in `next.config.js`). Manual `React.memo()`, `useMemo()`, and `useCallback()` are generally unnecessary — the compiler handles memoization automatically. Rules mentioning these can be skipped unless dealing with edge cases.
> - This project uses **tRPC + TanStack React Query** for data fetching, not SWR. Sections referencing SWR (4.1, 4.3) should be adapted to use tRPC patterns instead.
> - **Turbopack** is enabled for dev server (`next dev --turbopack`).
