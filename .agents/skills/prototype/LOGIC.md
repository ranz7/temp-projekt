...
Because it's one file with nothing to install, you can hand it to a non-developer — a designer, a mentor, a domain expert — and let them feel the model for themselves. So it speaks their language (Polish domain terms from baza wiedzy where they exist), not the code's.
...
Put the actual logic — the bit that's answering the question — in a single `<script>` block written as a small, pure module that could be lifted out and dropped into the real codebase later (it will become a TypeScript module; write it so the port is mechanical). The page around it is throwaway; this module isn't.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. The logic-specific mapping: the validated reducer / machine / function set lifts into the real TypeScript module (the decision, absorbed); the HTML shell rides along to the `prototype/<topic>` branch that keeps the prototype as a primary source — and being one self-contained file, it stays trivially re-runnable there.
